...
export default function SuccessCard(props) { 
  ...
  return (
    <article className={`mb-4 rounded-lg border border-gray-lightish shadow-sm w-full max-w-md ${props.justAddedNewStudy ? 'block' : 'hidden'} ${props.showingForm ? 'hidden' : 'block'}`}>
      ...
      <section className="bg-gray-faint border-t border-gray-lighter rounded-b-lg p-4 text-gray">Thanks for sharing! We'll add your case study after it's screened for bad stuff. Check back in a bit.</section>
    </article>
  );
}
